...
class mppi:
    ...
    def running_cost(self, state, action, prev_action, xh, uh, sig):
        ...
        ## Update the safety bound based on inferred rationality coefficient
    ...
    def updateTheta(self, theta_now, uH, uH_A, uH_D, u_A_sigma, u_D_sigma):
        ...
        x_next_sig_a = u_A_sigma/(self.beta.beta_a)
        x_next_sig_d = u_D_sigma/(self.beta.beta_d)
    ...
    def solve_mppi(self, state,  RH, ilq_results_A, ilq_results_D, theta_prob, beta_distr, t=None, Human_seq=None, learning=False):
        ...
        return xR.squeeze(0).cpu().numpy(), states.cpu().numpy(), self.U.cpu().numpy(), xH.cpu().numpy(), num_sampled_A
    ...
    def _compute_rollout_costs(self, perturbed_actions):
        ...
        for t in range(self.N):
            ...
            if not self.learning:
                ...
                if t < self.N-1:
                    self.theta_prob[t+1] = self.updateTheta(self.theta_prob[t], uH[:,t], u_A[:,t,:2], u_D[:,t], self.Sigma_A[:,t], self.Sigma_D[:,t] )
                ...
                if seq_len < self.N:
                    ...
                    if t == 0: 
                        ...
                        intention, intention_prob = self.encoder.get_drivingstyle(Human_seq[:,-self.N:].to(torch.float32))
        ...
        if any(c >= 10000): #If samples violate the constraints, set cost to NaN
