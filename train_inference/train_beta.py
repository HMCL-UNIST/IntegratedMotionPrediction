...
class MyDataSet(Dataset):
    def __init__(self):
        ...
        for i in range(num_data):
            ...
            sample, label = self.states_to_encoder_input_torch(states, u_h, 1.0,beta)
            self.samples.append(sample)
            self.output_labels.append(label)
        
        for i in range(num_data,num_data*2): #,num_data+num_data):
            attentive_data = np.load(file_name+'/distracted'+str(i)+'.npz',allow_pickle=True)
            states = attentive_data['traj']
            u_h = attentive_data['human_input']
            beta = attentive_data['t_beta']
            
            sample, label = self.states_to_encoder_input_torch(states, u_h, 0.0,beta)
            self.samples.append(sample)
            self.output_labels.append(label)
    ...
    def gen_dataset(self):
        ## Devide the data for training, validation, and test
    ...
    def states_to_encoder_input_torch(self, states, u_h, theta, beta):
       
        time_horizon = len(states)
        input_data_arr = torch.zeros(time_horizon, 11) #input_dim = 11
        for i in range(time_horizon):
            input_data_arr[i,:] = torch.tensor([ states[i][0],  #human x            
                            ...
                            theta])
        output_label_arr = torch.tensor([float(beta) for _ in range(time_horizon)])
        
        return input_data_arr, output_label_arr

class Encoder(nn.Module):
    def __init__(self,input_size = 11, hidden_size = 5, num_layers = 3):
...
class EncoderClassifier(nn.Module):
    def __init__(self, args, drop_out = 0):
        ...
        self.fc_logvar =  nn.Linear(self.hidden_size, self.output_size)
    ...
    def forward(self, x, intention_label=None):
        ...
        intention_mu = self.fc(enc_h)
        intention_mu = intention_mu* 0.8 + 0.2 #bata is ranged from 0.2 to 1
        intention_logvar = F.relu(self.fc_logvar(enc_h))

        if intention_label is None:
            return intention_mu, intention_logvar

        else:
            intention_label = intention_label.to(torch.float32)
            mu = intention_mu
            target = intention_label.reshape(len(intention_label),1)

            loss = -0.5 * torch.sum(intention_logvar - torch.exp(intention_logvar) - (target - mu)**2 / torch.exp(intention_logvar))
            accuracy = torch.sqrt(torch.mean((mu - target)**2))

            return loss, accuracy, intention_mu
...
class BetaClassifier():
    ...
    def model_load(self, model_id =None):
        ...
        saved_data = torch.load(self.file_dir+f"beta_encoder_{model_id}.model")            
    ...
    def model_save(self, model_id= None):
        if model_id is None:
            model_id = self.model_id
        save_dir = self.file_dir +f"beta_encoder_{model_id}.model" 
        torch.save({
    ...
    def get_beta(self, data):
        with torch.no_grad():
            intention_mu, intention_logvar = self.model(data)
        return float(intention_mu.item()), float(intention_logvar.item())
        ...
        for epoch in epochs:
            ...
            for i, batch_data in train_iterator:
                ...
                for j in range(num_sequences):
                    ...
                    loss, acc, _ = model(sequence, intention_label=label)
                ...
                acc_ = acc_total.item()/num_sequences
            ...
            accuracy = accuracy_itr/len(train_iterator)
            ...
            with torch.no_grad():
                for i, batch_data in test_iterator:
                    ...
                    accuracy_eval_itr += acc_
                
            eval_losses = eval_losses_itr/len(test_iterator)
            accuracy_eval = accuracy_eval_itr/len(test_iterator)
...
if __name__=="__main__":
    ...
    internalstate_encoder = BetaClassifier(args = args_)
